...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // "register" → /api/users/register  ← make sure this route exists on your backend
      const { data } = await api.post(`/api/users/${state}`, formData)
      dispatch(login(data))
      localStorage.setItem('token', data.token)
      toast.success(data.message)
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message) // ← .error() for red styling
    } finally {
      setLoading(false)
    }
  }
      ...
      <form onSubmit={handleSubmit} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
        ...
        {state === "login" && (
          <div className="mt-4 text-left text-green-500">
            <button className="text-sm" type="button">Forget password?</button> {/* ← type="button" prevents form submit */}
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={loading}
          className="mt-2 w-full h-11 rounded-full text-white bg-green-500 hover:opacity-90 transition-opacity disabled:opacity-60"
        >
